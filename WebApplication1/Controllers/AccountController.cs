...
namespace WebApplication1.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(IEmailService emailService, INotificationService notificationService)
        {
                ...
                try
                {
                    ...
                    if (_authService != null)
                    {
                        ...
                        if (_authService.Register(user, model.Password))
                        {
                            ...
                            try
                            {
                                debugInfo.AppendLine("Attempting to send confirmation email...");
                                var confirmationToken = Guid.NewGuid().ToString(); // Generate a unique token
                                await _emailService.SendEmailConfirmationAsync(user, confirmationToken);
                                debugInfo.AppendLine("Confirmation email sent successfully.");
                            }
                            catch (Exception emailEx)
                            {
                                debugInfo.AppendLine($"Failed to send confirmation email: {emailEx.Message}");
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ForgotPassword(WebApplication1.ViewModels.ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    var resetToken = _authService.GeneratePasswordResetToken(user.UserId); // Generate a unique token for the user
                    var resetLink = Url.Action("ResetPassword", "Account", new { userId = user.UserId, token = resetToken }, Request.Url.Scheme);
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            {
                var user = _authService.GetCurrentUser();
                if (user != null && await _authService.ChangePassword(user.UserId, model.CurrentPassword, model.NewPassword))
                {
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            if (await _authService.ResetPassword(user.UserId, model.Token, model.NewPassword))
            {
