...
namespace WebApplication1.Services
{
    public class AuthService
    {
        ...
        public async Task<string> GeneratePasswordResetToken(int userId)
        {
            ...
            user.PasswordResetToken = PasswordHelper.HashPassword(token); // Hash the token before storing
        ...
        {
            var user = await _db.Users.FindAsync(userId);
            if (user == null || string.IsNullOrEmpty(user.PasswordResetToken) || user.PasswordResetTokenExpiry <= DateTime.UtcNow)
            {
                return false; // User not found, token not set, or token expired
            }

            if (PasswordHelper.VerifyPassword(token, user.PasswordResetToken))
            {
                user.PasswordHash = PasswordHelper.HashPassword(newPassword);
